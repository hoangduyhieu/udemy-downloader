...
def mux_process(video_title, video_filepath, audio_filepath, output_path):
    ...
    if os.name == "nt":
        if use_h265:
            ...
            command = 'ffmpeg -loglevel panic -y -i "{}" -i "{}" -c:v copy -c:a copy -fflags +bitexact -map_metadata -1 -metadata title="{}" "{}"'.format(
        ...
        if use_h265:
            ...
            command = 'nice -n 7 ffmpeg -loglevel panic -y -i "{}" -i "{}" -c:v copy -c:a copy -fflags +bitexact -map_metadata -1 -metadata title="{}" "{}"'.format(
                video_filepath, audio_filepath, video_title, output_path
            )
